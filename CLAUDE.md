...
- **Une seule méthode de connexion pour tout le monde, identifiant + mot de passe** (`app/login/actions.ts`) — pas de magic link, pas d'email envoyé par l'app. Simplifie le déploiement (aucun SMTP/domaine à configurer côté Supabase) et évite qu'un athlète ait à gérer un email pour ça.
  - Pour un athlète, l'identifiant s'appuie sur un compte Supabase Auth classique avec un email interne synthétique (`identifiant@athlete.appcoaching.internal`, jamais résolu ni envoyé) et un mot de passe = le code défini par le coach — pas un système d'auth parallèle. `athlete.identifiant` sert aussi de slug d'URL admin (`/admin/athletes/[identifiant]`, au lieu de l'UUID) — obligatoire et unique pour tout athlète, que le coach configure ou non des identifiants de connexion dessus.
  - Pour le coach, l'"identifiant" est directement son adresse email réelle (compte Auth déjà existant, pas de slug ni de table dédiée — un seul coach, pas besoin d'une abstraction pour un cas qui ne peut pas encore arriver).
  - `signIn` (app/login/actions.ts) distingue les deux uniquement par la présence d'un `@` dans le champ identifiant : un `athlete.identifiant` ne peut jamais en contenir (`IDENTIFIANT_PATTERN`), donc aucune ambiguïté ni requête supplémentaire nécessaire.
